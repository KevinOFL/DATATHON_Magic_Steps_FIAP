...
class pre_processing:
    ...
    def exclude_columns(df, columns_to_exclude: list):
        ...
        df_copy = df_copy.drop(columns=columns_to_exclude, errors="ignore")
        return df_copy
    ...
    def padronize_collumns(df):
        ...
        df_copy.columns = (
            df_copy.columns.str.normalize("NFKD")
    ...
    @staticmethod
    def create_columns_for_discrepancies_in_subjects(df):
        """
        Cria novas colunas que representam a diferença entre as notas de cada matéria
        e a mediana dessas notas, para as matérias de matemática, português e inglês.

        parametros:
        - df (pd.DataFrame): O DataFrame original.

        retorno:
        - pd.DataFrame: Um novo DataFrame com as colunas de diferença adicionadas.
        """
        df_copy = df.copy()

        median_mat = df_copy["mat"].median()
        median_por = df_copy["por"].median()
        median_ing = df_copy["ing"].median()

        df_copy["diferenca_mat"] = df_copy["mat"] - median_mat
        df_copy["diferenca_por"] = df_copy["por"] - median_por
        df_copy["diferenca_ing"] = np.where(
            df_copy["estudou_ingles"] == 1, df_copy["ing"] - median_ing, 0
        )
        return df_copy

    @staticmethod
    def create_column_for_discrepancie_in_ieg_inde(df):
        """
        Cria novas colunas que representam a diferença entre as notas
        dos paramêtros de avaliação interna, IEG e INDE.
        ...
        median_ieg = df_copy["ieg"].median()
        median_inde = df_copy["inde"].median()

        df_copy["diferenca_ieg"] = df_copy["ieg"] - median_ieg
        df_copy["diferenca_inde"] = df_copy["inde"] - median_inde
